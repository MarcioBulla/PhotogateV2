...
static char *print_option_centripetal(uint8_t selected) {
  char option_str[22];
  ...
  snprintf(hist_str, sizeof(hist_str), "Cen%02u", selected % 100);
  return hist_str;
}

static counter_config_t mk_counter_config_centripetal(uint8_t value) {
  ...
  ESP_LOGI(TAG, "Counter config created");

  return counter_config;
}

void fMenu_centripetal(void *args) {
  experiment_config_t centripetal = {
      .title = "Centripetal",
      .default_value = CONFIG_CENTRIPETAL,
      .interval_options = {1, 99},
      .check_obstruct = true,
      .print_option = &print_option_centripetal,
      .mk_counter_config = &mk_counter_config_centripetal,
  };

  run_experiment(centripetal);
}
